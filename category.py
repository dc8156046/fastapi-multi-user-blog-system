...
async def create_category(request: CategoryCreate, db: db_dependency, user: user_dependency):
    category = Category(**request.dict(), user_id=user.id, created_at=datetime.now(), updated_at=datetime.now())
    db.add(category)
    db.commit()
    db.refresh(category)
    return category
...
async def update_category(category_id: int, request: CategoryUpdate, db: db_dependency, user: user_dependency):
    category = db.query(Category).filter(Category.id == category_id, Category.user_id == user.id).first()
    if not category:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Category not found")
